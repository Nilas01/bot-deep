...
# Remplace METS_TON_TOKEN_ICI par le vrai jeton fourni par @BotFather
TOKEN = "8402606079:AAE74zmoeXlBehpOOsRAa2O3OkyM0ze1wSA"

# Base de données pour le Quiz
QUIZ_DATA = [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    texte = (
        "🤖 Bienvenue sur DEEP 2.0 (Audio & Images OK) !\n\n"
        ...
        "• 🎨 *Générer une image* : Écris 'imagine' suivi de ton idée (Ex: imagine un chat astronaute).\n"
        "• 📄 *Photo en PDF* : Envoie-moi une photo normale pour obtenir un PDF.\n"
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔍 DEEP Activé ! Pose-moi ta question par texte ou par note vocale 🎙️.")
...
async def transcrire_vocal_direct(audio_bytes: bytes) -> str:
    try:
        # Envoi direct du flux OGG Telegram à l'API Wit.ai (configuré en audio/ogg)
        headers = {
...
async def gerer_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⏳ Traitement de ton image... Création du PDF...")
    try:
        fichier_photo = await update.message.photo[-1].get_file()
        photo_bytes = await fichier_photo.download_as_bytearray()
        image = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
        pdf_io = io.BytesIO()
        image.save(pdf_io, "PDF", resolution=100.0)
        pdf_io.seek(0)
        await update.message.reply_document(document=pdf_io, filename="DEEP_Document.pdf", caption="🎉 Ton PDF est prêt !")
    except Exception as e:
        logger.error(f"Erreur PDF : {e}")
        await update.message.reply_text("❌ Échec de la création du PDF.")
...
async def gerer_vocal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_data.get('search_active', False):
        await update.message.reply_text("⚠️ Appuie d'abord sur /search avant de m'envoyer un message ou un vocal !")
        return
    ...
    try:
        ...
        if not texte_transcrit:
            await update.message.reply_text("❌ Je n'ai pas bien compris. Parle plus fort et distinctement s'il te plaît !")
            return
            
        await update.message.reply_text(f"🗣️ *Tu as dit :* \"_{texte_transcrit}_\"\n\n⚡ DEEP lance la recherche IA...", parse_mode="Markdown")
        
        # Recherche IA à partir du texte extrait du vocal
        ...
        logger.error(f"Erreur général vocal : {e}")
...
async def gerer_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if texte_clean.startswith("imagine") or texte_clean.startswith("génère") or texte_clean.startswith("genere"):
        await update.message.reply_text("🎨 Dessinement en cours...")
        ...
        if image_bytes: 
            await update.message.reply_photo(photo=image_bytes, caption=f"✨ Pour : '{prompt_image}'")
        else: 
            await update.message.reply_text("❌ Erreur lors du dessin.")
        return
...
def main():
    print("🚀 DEEP V2.0 FINALISÉ - EN COURS...")
